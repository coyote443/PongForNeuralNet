...
{
    ui->setupUi(this);
    m_PongModel         = new PongModel(WIDTH, HEIGHT, this);
    ...
    ui->actionSI->setIcon(QIcon(":/icos/SI.ico"));
    ...
    connect(m_PongModel,    SIGNAL(sbGetPoints()),  this, SLOT(resetTimeStuff()) );
...
void MainWindow::checkMusicLoop()
{
    if(m_BackgroundMusic.isFinished() && m_IsBackgroundPlay)
        m_BackgroundMusic.play();
}

void MainWindow::checkSIMovement()
{
    if(m_IsSIPlay == true){
        m_PongModel->movePaddle(COMPUTER, 0);
        m_PongModel->refresh();
    }
}
...
void MainWindow::on_actionMakeNoise_triggered(){
    m_PongIsRunning = false;
    int howManyWalls = QInputDialog::getInt(0, "Noise", "How many walls?", 0, 0, 100);
...
void MainWindow::on_actionStop_Music_triggered(){
    if(m_IsBackgroundPlay){
    ...
    m_IsBackgroundPlay = !m_IsBackgroundPlay;
}

void MainWindow::on_actionSI_triggered(){
    m_IsSIPlay = !m_IsSIPlay;
}
